...
[`shared/`](shared) is the single source of truth. The [`chrome/`](chrome) and
[`firefox/`](firefox) folders are assembled from it, so **run the build once** to
populate them with real files (Chrome won't load a folder whose files are
...
```
shared/        ← single source of truth, the only committed copy (edit here)
  content.js   content.css   popup.html   popup.js   icons/
chrome/
  manifest.json            (committed)
  content.js, …, icons/    (generated by scripts/build.sh; git-ignored)
firefox/
  manifest.json            (committed; adds browser_specific_settings.gecko)
  content.js, …, icons/    (generated; git-ignored)
```

Only each `manifest.json` is committed per browser; everything else is copied in
from `shared/` by `scripts/build.sh`. We copy real files rather than symlink
because Chrome refuses to load an unpacked extension whose files resolve outside
its root directory (an earlier symlink-based layout broke for exactly this
reason). The same `chrome.*` (callback-style) APIs and clipboard code work in
both browsers, so only the manifests differ.
